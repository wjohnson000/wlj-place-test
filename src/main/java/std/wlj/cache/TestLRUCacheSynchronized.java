...
import std.wlj.cache.lru.ExpiringLRUCache;
import std.wlj.cache.lru.LRUCache;

public class TestLRUCacheSynchronized {

//    static  LRUCache<String, MyObject> myCache = new LRUCache<>("name", 10_000);
    static  ExpiringLRUCache <String, MyObject> myCache = new ExpiringLRUCache<>("name", 10_000);
        ...
        for (int i=0;  i<threads.length;  i++) {
            threads[i] = new Thread(new Runnable() {
                @Override public void run() {
                    ...
                    System.out.println("LRU CACHE (SYNCHRONIZED) STATISTICS ...");
                    System.out.println("---------------------------------------");

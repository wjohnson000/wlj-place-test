...
public class AnalysisShimAsyncThreadLocal {
    ...
    protected static final Logger logger = new Logger(AnalysisShimAsyncThreadLocal.class);
    ...
    private static ThreadLocal<CloseableHttpAsyncClient> httpClients =
        new ThreadLocal<CloseableHttpAsyncClient>() {
            @Override
            protected CloseableHttpAsyncClient initialValue() {
                clientCnt.increment();
...
System.out.println("IN-OUT: " + inOut.longValue());
        ...
        CloseableHttpAsyncClient client = httpClients.get();

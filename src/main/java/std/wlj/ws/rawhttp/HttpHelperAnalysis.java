...
public class HttpHelperAnalysis {
    ...
    private static RootModel doRequestWithEntity(URL url, RootModel model, String method) throws Exception {
        ...
        if (urlConn instanceof HttpURLConnection) {
            ...
            String contentType = httpUrlConn.getRequestProperty("Content-Type");
            if (contentType == null  ||  contentType.toLowerCase().contains("xml")) {
                out.write(toXML(model));
            } else {
     ...
     * @return JSON representation of the object
     */
    private static String toXML(RootModel model) {
        try {
            xmlMarshaller.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);

            StringWriter writer = new StringWriter();
            xmlMarshaller.marshal(model, writer);
            return writer.toString();
        } catch (JAXBException e) {
            System.out.println("Unable to unmarshal XML: " + e.getLocalizedMessage());
        }

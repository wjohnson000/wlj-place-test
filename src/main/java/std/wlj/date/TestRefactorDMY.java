        ...
        for (LocalizedDateFormat ldf : LDF) {
            for (StdLocale locale : LOCALE) {
                for (int cnt=0;  cnt<200;  cnt++) {
                    ...
                    Dates      datesOld = dateOld.getDatesDMY(yrr, mon, day, ldf, locale);
                    DateResult dateRes = dateNew.getDatesDMY(yrr, mon, day);
                    ...
                    System.out.println(yrr + "|" + mon + "|" + day); 
                    if (! xmlOld.trim().equals(xmlNew.trim())  &&  yrr > 100) {
                        badCount++;
                        System.out.println("\n\nOLD:\n" + xmlOld);
                        System.out.println("\nNEW:\n" + xmlNew);
                    }

 ...
 *         <p/>
 *         Copyright(c) 2005 Intellectual Reserve Inc. All rights reserved. Unauthorized reproduction of this software is prohibited and is in violation of
 *         United States copyright laws.
 */

// The Korean traditional calendar called Dan-Ki (or Tan-Ki)
// The Tangun era (Tan-Ki) began with the founded ancient Chosun in 2333 BC.
// see Korean Myths and Tales : Tangun, The Father of Our Nation
...
public class KoreanDanKiCalendar extends KoreanCalendar {
  ...
  public AstroDay dayFromDayMonthYear(int d, int m, int y, int i) {
    ...
    // assume Gregorian rules after the adoption of the Gregorian
    // calendar; use the CJK Lunar calendar before.
    y = convertTangunEraYearToChristianEra(y);// convert to the Christian Era
    if (y < firstGregorianYear() || i != 0) {
      return super.dayFromDayMonthYear(d, m, y, i);
    }
    return GregorianCalendar.getInstance().dayFromDayMonthYear(d, m, y);
  }
   ...
   * @return the Christian equavlent of the parameter year
   */
  private static int convertTangunEraYearToChristianEra(int year) {
    // Tangun year is always positive
    year -= 2333;// subtract Tangun origin
  ...
  private static int convertChristianEraYearToTangunEra(int year) {
    // convert year to internal format for calculations
    if (year < 0) {
      year++;// convert 1 BC to 0, 2 BC to -1, etc.
    }
    year += 2333;// add Tangun origin
    // Tangun year is always positive
    return year;
  }

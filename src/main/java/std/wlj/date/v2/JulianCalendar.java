...
/**
 * The Julian Calendar was instituted on Jan 1, 45 BC, (709 A.U.C. [ab urbe condita] or [anno urbis
 * conditae], the traditional founding of Rome in 754 BC)
 * 
 * @author Pete Blake
...
public class JulianCalendar implements Calendar {
    ...
    public boolean isLeapYear(int year) {
        // At the inception of the Julian calendar in 45 BC, every fourth year was supposed to be
        // a leap year.  Historical evidence indicates that instead, every third year was taken as
        // a leap year from 45 BC to 9 BC, and to correct the error, leap years were omitted until
        // 8 AD. (skipped: 5 BC, 1 BC, 4 AD).
        int y = year;
        if (y >= 8) {
        ...
        // y < 0, make 1 BC == 0, 2 BC == -1, etc.
        y++;
        if (y > -8) {
            return false; // no leap years from 8 BC thru 1 BC
        } else if (y > -44) { // find third years from 45 BC to 9 BC
            return (y % 3) == 0;
        } else {
            return (y % 4) == 0;  // revert to 'normal' for years before 45 BC, (proleptic Julian calendar)
        }
    }
    ...
    public AstroDay dayFromDMY(DMY dmy, int i) {
        // ignore the intercalary month i, which is used in CJK lunar calendars
    ...
    public AstroDay dayFromDayMonthYear(int day, int month, int year) {
        int d = day;
        int m = month;
        int y = year;
        ...
        if (0 == d  &&  0 == m  &&  0 == y) {
            return AstroDay.INVALID;
        } else if (y < -4001  ||  y > 3000) {
            return AstroDay.INVALID;
        } else if (y == 0) { // special case for missing year
            return day(d, m);
        }
        ...
        // renumber months so March is month 0, then use the nifty formula below.  Jan and Feb will
        // be months 10 and 11 in the previous year.
        if (m > 2) {
            m -= 3;
        } else {
            m += 9;
    ...
    protected AstroDay day(int d, int m, int y) {
        ...
        // Mar is month 0, Jan and Feb are month 10 and 11 in the previous year, and the other month
        // numbers are adjusted accordingly

        // Note that calculations here follow the rules of the calendar as they were specified by
        // Sosigenes and decreed by Julius Caesar.  The muddling of the leap years by the pontifices
        // from 45 - 9 BC is NOT accounted for here. (But see isLeapYear).
    ...
    public AstroDay day(int day, int month) {
        int d = day;
        int m = month;
    ...
    public DMY dm(AstroDay day) {
        ...
        } else if (day.value() > 367) {
            return dmy(day);
        }

        // Handle day values between 1 and 367, i.e., missing year
        int m = 0;

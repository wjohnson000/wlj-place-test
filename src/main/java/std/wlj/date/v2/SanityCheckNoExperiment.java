 ...
 *
 */
public class SanityCheckNoExperiment {
...
//        datesToParse.add("道武帝皇始三年七月七日");
    ...
    static boolean noInterp(List<InterpResult> beta, List<InterpResult> local) {
        if (beta.isEmpty()  &&  local.isEmpty()) {
            return true;
        }
        return false;
    }

    static boolean allAssisted(List<InterpResult> beta, List<InterpResult> local) {
        if (beta.size() > 0  &&  beta.get(0).isAssisted   &&  local.get(0).isAssisted) {
            return true;
        }
        return false;
    }

    static boolean noneAssisted(List<InterpResult> beta, List<InterpResult> local) {
        if (beta.size() > 0  &&  !beta.get(0).isAssisted  &&  !local.get(0).isAssisted) {
            return true;
        }
        return false;
    ...
    static boolean v2MatchV1(List<InterpResult> beta, List<InterpResult> local) {
        if (beta.size() > 0  &&  local.size() > 0  &&  beta.get(0).isAssisted  &&  ! local.get(0).isAssisted  && beta.get(0).gedcomx.equals(local.get(0).gedcomx)) {
            return true;
        }
        return false;
    }

...
/**
 * Calendar: a system for fixing the beginning, length, and divisions of the year and arranging the
 * days and longer divisions of time (weeks, months) in a definite order. (From Merriam-Webster's
 * Collegiate Dictionary).  Calendars usually have some relation to, and try to model, the natural
 * order of the earth, moon and sun.  Often a religion and a calendar are closely tied. Some cultures
 * use multiple calendars simultaneously: civil, religious, historical.
 * <p/>
 * n.b. The YEAR value is always relative to the Christian Era, and zero is not a valid value. In the
 * Christian Era, BC years are negative: -1 == 1BC, -2 == 2BC, etc., and AD years are positive: 1, 2, 3.
 *
 * @author Pete Blake
...
public interface Calendar {
    ...
    /**
     * Return the year from a AstroDay. The value returned is always relative to the Christian Era,
     * and zero is not a valid value.  BC years are negative: -1 == 1BC, -2 == 2BC, etc. and AD years
     * are positive: 1, 2, 3.
     *
    ...
    /**
     * Calculate a new AstroDay from a AstroDay and a number of years, taking leap years into account,
     * according to the rules of the calendar.  For example, to calculate a jday for age 8, give the
     * AstroDay for the birthday and 8 years. The result will be the AstroDay for the 8th birthday.
     * The calculation accounts for leap years.
     *
     ...
     *
     * @param year and integer value for the year
     * @return TRUE if y is a valid year in this calendar Note that all tables and calculations
     *         use the Christian Era year.  But raw years entered by patrons maybe from other eras,
     *         e.g. the Tangun Era.
     */
     ...
     * @param day an integer value (1-31) for a day of the month
     * @param month an integer value (1-12) for the month of the year
     * @param year the Christian Era year in external form: 1 BC == -1, 2 BC == -2, &etc
     ...
     * @param y the Christian Era year in external form: 1 BC == -1, 2 BC == -2, &etc

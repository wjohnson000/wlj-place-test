 ...
 *
 */
public class PlaceAndRepTestNew {
    ...
    public static void main(String[] args) throws Exception {
        DataSource ds = DbConnectionManager.getDataSourceWLJ();
        SolrService       solrService = SolrManager.localEmbeddedService("D:/solr/newbie-6.1.0");
    ...
    private static void createPlacesAndReps() {
        ...
        try {
            ...
            PlaceRepBridge placeRepB01R = dbRService.getRep(placeRepB01.getRepId());
            PlaceRepBridge placeRepB02R = dbRService.getRep(placeRepB02.getRepId());
            PlaceRepBridge placeRepB03R = dbRService.getRep(placeRepB03.getRepId());
            PlaceRepBridge placeRepB04R = dbRService.getRep(placeRepB04.getRepId());
            PlaceRepBridge placeRepB05R = dbRService.getRep(placeRepB05.getRepId());
            System.out.println("Made it here, so far, so good ...");

            // Update a place
            PlaceBridge    placeB01U    = updateQuillyPlace(placeB01a.getPlaceId());
            System.out.println("Made it here, so far, so good ... more and more betterer ...");
            PlaceBridge    placeB01Ra   = dbRService.getPlace(placeB01U.getPlaceId());

            // Update a place-rep
            PlaceRepBridge placeRepB03U  = updateNerfPlaceRep01(placeRepB03.getRepId(), placeB03a.getPlaceId(), placeRepB02.getRepId());
            PlaceRepBridge placeRepB03UR = dbRService.getRep(placeRepB03U.getRepId());
            ...
            PlaceRepBridge placeRepB03DX = dbRService.getRep(placeRepB03.getRepId());
            PlaceRepBridge placeRepB04DX = dbRService.getRep(placeRepB04.getRepId());
    ...
    private static PlaceRepBridge createQuilly() throws PlaceDataException {
        PlaceRepBridge prb = dataService.createPlace(
            -1,
            1900,
            null,
            281,
    ...
    private static PlaceBridge updateQuillyPlace(int placeId) throws PlaceDataException {
        PlaceBridge pb = dataService.updatePlace(
            placeId,
            1925,
            null,
    ...
    private static PlaceRepBridge createNerfRep02(int placeId, int parentId) throws PlaceDataException {
        PlaceRepBridge prb = dataService.createRep(
            placeId,
            parentId,
            1925,
            1968,
    ...
    private static PlaceRepBridge updateNerfPlaceRep01(int repId, int placeId, int parentId) throws PlaceDataException {
        PlaceRepBridge prb = dataService.updateRep(
            repId,
            placeId,
            parentId,
            1910,
    ...
    private static void output(PlaceRepBridge placeRepB) {
        if (placeRepB == null) {
            ...
            System.out.println("REP: " + placeRepB.getRepId() + "|" + niceJurisdiction(placeRepB.getJurisdictionIdentifiers()) + "|" + 
                ...
                placeRepB.getCreateDate() + "|" + placeRepB.getLastUpdateDate() + "|" + 
